...
class Exportacion(models.Model):
    ...
    def _receptor(self):
        ...
        Receptor['RUTRecep'] = self.format_vat(commercial_partner_id.document_number)
        ...
        if not self._es_boleta():
            GiroRecep = commercial_partner_id.activity_description.name
    ...
    def get_monto_clausula(self):
        test = self.payment_term_id.with_context(currency_id=self.currency_id.id).compute(
            self.amount_total, date_ref=self.date_invoice)
    ...
    def _bultos(self, bultos):
        ...
        for b in bultos:
            ...
            Bultos.append(Bulto)
        Bultos[0]['CantBultos'] = cant_bultos
        return [Bultos[0]]
