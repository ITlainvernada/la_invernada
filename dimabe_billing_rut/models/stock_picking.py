...
class StockPicking(models.Model):
    ...
    def send_to_sii(self):
        ...
        for item in move_line:
            ...
            price_unit = item.sale_id.mapped('order_line').filtered(lambda
                                                                        a: a.product_id.id == item.product_id.id).price_unit if self.is_multiple_dispatch else item.picking_id.sale_id.mapped(
                'order_line').filtered(lambda a: a.product_id.id == item.product_id.id).price_unit
            ...
            if len(self.sale_id.mapped('order_line').filtered(lambda a: a.product_id.id == item.product_id.id).mapped(
                    'tax_id')) == 0:
                haveExempt = True
                ...
                productLines.append(
                    {
                        ...
                        "ProductQuantity": str(item.real_dispatch_qty) if self.is_multiple_dispatch else str(
                            item.quantity_done),
                        ...
                        "DiscountAmount": "0",
                        "Amount": str(self.roundclp(
                            item.real_dispatch_qty * price_unit if self.is_multiple_dispatch else item.quantity_done * price_unit)),
                        # str(int(amount)),
        ...
        invoice = {
            ...
            "transmitter": {
                "EnterpriseRut": re.sub('[\.]', '', self.company_id.invoice_rut),
